...
int main(int argc, char *argv[])
{
    ...
    RoboCupSSLServer *server = new RoboCupSSLServer(10007,"224.5.23.2");
    RoboCupSSLClient client;

    if(!client.open(true)){
	printf("Erro ao abrir o client da visao!\n");
    	return 0;
    }

    if(!server->open()){
	printf("Erro ao abrir o servidor da visao!\n");
    	return 0;
    }
    ...
    while(true) {
        ...
        if (client.receive(packet)) {
            ...
            if (packet.has_detection()) {
                ...
                if(!server->sendWrapperPacket(packet)){
                  printf("\nErro ao enviar o pacote!\n");
                }else printf("\nSucesso ao enviar o pacote!\n");
